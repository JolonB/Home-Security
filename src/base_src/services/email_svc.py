...
import smtplib, imaplib, email, ssl
import sys

sys.path.append(".")  # TODO there must be a better way to do this
...
def _get_email_config():
    return config_secret.email["address"], config_secret.email["password"], config_secret.email["port"], config_secret.email_recipients
...
def _gen_valid_recipients(recipients:list) -> str:
    ...
    return "(OR {} {})".format(_gen_valid_recipients(first_half), _gen_valid_recipients(second_half))


def _parse_msg(parsedbytes:email.message) -> dict:
    msg_full = parsedbytes.get_payload()[0].get_payload() if parsedbytes.is_multipart() else parsedbytes.get_payload()
    msg_full = msg_full.split('\r\n',1)
    return {'from': parsedbytes.get('From'), 'to': parsedbytes.get('To'), 'subject': parsedbytes.get('Subject'), 'header': msg_full[0], 'body': msg_full[1]}
...
def _rec_email():
    ...
    result, data = mail.search(None, "UNSEEN", _gen_valid_recipients(list(recipients.keys())))
