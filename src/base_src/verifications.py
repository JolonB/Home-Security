...
for addr, key in config_secret.email_recipients.items():
    ...
    assert isinstance(key, int), "key {} in `email_recipients` is not integer".format(
        key
    )
...
for phone, key in config_secret.mobile_recipients.items():
    ...
    assert isinstance(key, int), "key {} in `mobile_recipients` is not integer".format(
        key
    )
...
for key, values in config_secret.user_info.items():
    ...
    contains_keys(
        "user_info[{}]".format(key), values, ("rank", int), ("pin", str), ("salt", str)
    )
    assert (
